 ...
 * `models`, in order, via the configured provider's OpenAI-compatible
...
export async function generateWithFallback(config: OpenAiCompatibleConfig, params: any, models: string[]): Promise<any> {
  ...
  for (const model of models) {
    try {
      observation.logTelemetry("info", "Cognition", `Attempting OpenAI-compatible content generation with model: ${model}`);
      const res = await fetchWithRetry(
        ...
        { label: `OpenAI-compatible chat/completions (${model})` }
      );
      if (!res.ok) {
        const body = await res.text().catch(() => "");
        throw new Error(`OpenAI-compatible endpoint returned ${res.status}: ${body}`);
      }
      const data = await res.json();
      observation.logTelemetry("info", "Cognition", `Successfully generated content with model: ${model}`);
      return data;
    } catch (error: any) {
      lastError = error;
      observation.logTelemetry("warn", "Cognition", `Model ${model} failed: ${error.message || error}`);
    }
  }

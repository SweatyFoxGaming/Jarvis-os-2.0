...
export async function runDailyAdaptation(username = "admin"): Promise<{ ok: boolean; reportPath: string; candidateObjectiveStarted: boolean }> {
  ...
  try {
    ...
    let reflectionText = "No OmniRoute client configured — unable to generate a written reflection today.";
    let capabilityGaps: string[] = [];
    let candidateObjective = "";

    if (configuredOmniRoute) {
      const objectivesSummary = objectives.length > 0
        ? objectives.map(o => `- ${o.description}`).join("\n")
        : "(no active objectives)";

      // Guarded independently of the outer try/catch: an OmniRoute outage
      // here must degrade to the same placeholder text used when OmniRoute
      // isn't configured at all, not skip the report/notification entirely —
      ...
      try {
        ...
        observation.logTelemetry("warn", "Adaptation", `OmniRoute reflection call failed: ${err.message} — falling back to a signals-only report.`);
        reflectionText = `OmniRoute reflection call failed today (${err.message}) — this report reflects only the computed analyzer signals below.`;
      }
    }

...
3. **Cognition router** (`src/runtime/cognition-router.ts`) — the new thing every call site actually talks to. `CognitionRouter.generateWithFallback(username, params, models)`:
   ...
   - If every key across every configured provider is genuinely exhausted, falls back to the existing local `LocalCognitiveEngine` (llama-cpp), returning its response in the same OpenAI-compatible shape callers already expect, and logs this as a real observability event (reusing `ObservationPlatform.logTelemetry`) so local-fallback frequency is visible — no silent quality degradation.
...
Call site (e.g. `departments.runResearch(objective, router, username)`) → `router.generateWithFallback(username, params, models)` → fair-share check against `usage-repo` → pick provider + available key from `key-pool` → attempt via `openai-compatible-client.ts`'s `generateWithFallback` against that provider's real base URL → on success, record usage, return the response → on 429/quota error, report failure to `key-pool` (cooldown), retry next key/provider → if every key across every provider is exhausted, fall back to `LocalCognitiveEngine`, log the fallback, return its response in the same shape.
...
- `cognition-router.ts`: tests using injected fake key pool / usage tracker / local-engine (dependency injection — the same pattern already established this session for `shadow-verifier.ts`'s `execFn` parameter), asserting: normal-capacity requests aren't throttled, an over-share user under a strained pool is delayed not rejected, a 429 triggers cooldown-and-retry with the next key, and full exhaustion falls through to the local engine with a logged event. No real network calls in automated tests.

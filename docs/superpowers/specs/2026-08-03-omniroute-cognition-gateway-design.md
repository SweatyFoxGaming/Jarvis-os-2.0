...
## Out of Scope: Voice-Native Mode and Embeddings
...
**Semantic memory embeddings are also excluded.** `src/cognition/memory-store.ts`'s `embedText`/`recall`/`remember` call Gemini's `ai.models.embedContent` — a third, structurally distinct API surface (vector embeddings, not chat completions) with its own already-working local fallback (an Ollama `/api/embeddings` endpoint). This stays on direct Gemini SDK usage, unchanged, same reasoning as voice-native mode: a different API shape than what this migration's `generateWithFallback` client targets, and not something the user asked to change. `src/cognition/memory-store.ts` is **not** touched by this plan.

**Practical consequence:** `getAi()` (the Gemini `GoogleGenAI` client) is **not removed** — it stays constructed exactly as today and continues to be threaded through to `executeTool`, `live-voice.ts`, and `memory-store.ts` for voice-native and embeddings use. Only Groq's client is fully replaced (all of its usage is chat-completions-shaped, with no embeddings/voice equivalent), and only the three Gemini chat-completions call sites in `server.ts` (listed below) move onto the new client. `src/capabilities/tools.ts`'s `executeTool` keeps its `ai: GoogleGenAI | null` parameter unchanged in type — it's still needed for the embedding/voice-adjacent code paths reachable through tool execution.
...
| `src/runtime/clients.ts` | Modify — add `getOmniRoute()`/`setSharedClient()` alongside the existing `getAi()` (kept, unchanged — still needed for embeddings/voice). `getGroq()`/`setSharedClients()`'s Groq half is removed; its callers switch to `getOmniRoute()` |
...
| `src/cognition/memory-store.ts` | **Not touched** — embeddings stay on direct Gemini SDK usage, out of scope (see above) |
| `src/capabilities/tools.ts` | **Not touched** — `executeTool`'s `ai: GoogleGenAI | null` parameter keeps its type unchanged, still passed a real `GoogleGenAI` instance from `server.ts` for embedding/voice-adjacent tool paths. `getAllToolDeclarations()` is unchanged (still produces Gemini's `Type`-tagged `FunctionDeclaration[]`, translated at the call site same as today's Groq path already does) |

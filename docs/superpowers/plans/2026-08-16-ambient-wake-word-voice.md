...
> **Scope note on that concurrency claim:** this removes the Node-level bottleneck (Task 3) that would otherwise serialize every session's cognition/reply regardless of the daemon; the voice daemon's own STT/TTS inference queue (`daemon/voice_engine.py`) is still globally shared across all connections and was explicitly out of scope for this plan (`No daemon-side changes` is a stated Global Constraint). Two users speaking at the same time therefore still queue behind each other at the daemon's inference layer — genuine concurrent-request audio processing would need a separate follow-up.

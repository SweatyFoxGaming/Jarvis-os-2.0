...
`analyzer.ts` moves from one level deep (`evolution/`) to one level deep (`adaptation/`) — same `../` counts, only segment names for already-moved targets change (e.g. `../observation/index.js` → `../kernel/observation.js`). `reflection.ts` and `long_term_learning.ts` move from `cognition/` (one level) to `adaptation/` (one level) — same rule applies (e.g. `reflection.ts`'s `./groq-client.js` sibling-style import, if present, becomes `../runtime/groq-client.js` once Task 7 has run, or stays `../cognition/groq-client.js` if this task runs before Task 7 — check `groq-client.ts`'s actual current location at the time this task executes and use whichever is correct).
